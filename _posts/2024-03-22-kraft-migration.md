---
layout: post
title:  "From ZooKeeper to KRaft: let's automate with Strimzi"
date: 2024-03-22
author: paolo_patierno
---

The Apache Kafka project has been using Apache ZooKeeper since its inception to store metadata.
Registered brokers, the current controller and topics configuration are just a few of the all data that ZooKeeper stores for a running Kafka cluster.
Using a dedicated and centralized system for maintaining configuration information sounded the right approach at that time and ZooKeeper has been doing its work for long.
...
TBD: put some console output of znode from ZooKeeper (i.e. brokers, controller, topics configuration)
...
{"version":1,"config":{"retention.ms":"10000"}} // note all other config params are using default so omitted

ZooKeeper is designed to be replicated across a number of nodes which built an ensemble.
A leader node is the one where all the write requests, by clients, are forwarded from other nodes and the operations are coordinated by using the ZooKeeper Atomic Broadcast (ZAB) protocol.
...
Because ZooKeeper requires a quorum, not having the majority of nodes to process requests can affect the Kafka cluster's availability as well.

ZooKeeper also represents a bottleneck for scalability and puts a limit on the number of topics and corresponding partitions supported within a Kafka cluster.
It has a performance impact when, for example, there is a controller failover and the new elected one has to fetch metadata from ZooKeeper, including all the topics information. Also, any metadata update needs to be propagated to the other brokers.
The problem is that the metadata changes propagations, by using RPCs, are linear with the number of topics' partitions involved and more partitions means more metadata to propagate, so it takes longer and the system gets slower.

TBD

### Welcome to KRaft!

In order to overcome the limitations related to the ZooKeeper usage, the Kafka community came with the idea of using Kafka itself to store metadata and using the event-driven pattern to take them updated across the brokers.
The work started with [KIP-500](https://cwiki.apache.org/confluence/display/KAFKA/KIP-500%3A+Replace+ZooKeeper+with+a+Self-Managed+Metadata+Quorum) in late 2019 by introducing a self-managed metadata quorum based on a specific Kafka implementation of the [Raft](https://raft.github.io/) consensus protocol. That was named KRaft!

KRaft is an event-based implementation of the Raft protocol and the quorum controller maintains an event log, a single-partition topic named `__cluster_metadata`, to store the metadata.
...
The leader broker of that single-partition topic is actually the controller node of the Kafka cluster.
The metadata changes propagation has the benefit of being event-driven via brokers replication and not using RPCs anymore.
The metadata management is directly within Kafka itself with the use of a new quorum controller service which uses an event-sources storage model.
The Kraft Raft (KRaft) consensus protocol is used to ensure that metadata are fully replicated across the quorum.

TBD: put a picture of KRaft quorum controller together with metadata topic

Removing an external system like ZooKeeper simplifies the overall architecture and remove the burden of taking care of an additional system.
Scalability is also improved by reducing the load on the metadata store by using snapshots to avoid unbounded growth (compacted topic).
When there is a leadership change in the quorum controller, the new leader already has all the committed metadata records so recovery is pretty fast.
...
When in mixed-mode, it gets both roles by handing Kafka clients communication on one hand and metadata changes on the other.
...
As today, it is expected to have users who are running ZooKeeper-based clusters but they are interested in migrating to KRaft mode as soon as possible in order to overcome all the limitations we have already talked about.
...
At the beginning, we have the Kafka brokers running in ZooKeeper-mode so connected to the ZooKeeper ensemble used to store metadata.

TBD: picture about ZooKeeper-based cluster
...
TBD: picture about KRaft controller quorum deployed
...
TBD: picture about brokers connected to KRaft as well and controllers running migration.
...
TBD: picture about dual-write mode and RPCs from controllers to brokers.
...
TBD: picture with brokers in full KRaft but controllers still connected to ZooKeeper
...
TBD: picture with cluster in full KRaft mode

#### Rollback support

During the migration process is also possible to execute a rollback procedure to revert the cluster using the ZooKeeper ensemble again.
...
There is a lot of manual intervention even related to verify the migration status by looking at the KRaft controller leader or checking the `ZkMigrationState` metric.
...
TBD: show Kafka custom resource using `strimzi.io/kraft` annotation (disabled for ZK and enabled for KRaft)
...
It is now possible to use two more values for that annotation: `migration` to start the migration process and `rollback` to revert the migration itself.
...
TBD: snippet of KafkaNodePool hosting controllers and `strimzi.io/kraft` with migration value
...
The Strimzi operator will run the following steps for you:
...
* checking the status of the migration and updating the `Kafka` custom resource to provide such status to the user;
* when the migration is done, the brokers are reconfigured to be not connected to ZooKeeper, with the migration flag disabled and are rolled again.
...
TBD: showing shell with all states during migration
...
In this status, the cluster is working in "dual-write" mode and the user can validate that everything is working fine before deciding to finalize the migration or rolling back to use ZooKeeper.
...
TBD: showing Kafka custom resource with `strimzi.io/kraft` as enabled and no running ZooKeeper nodes
...
TBD: showing Kafka custom resource with `strimzi.io/kraft` as rollback and shell with all states during rollback
...
After that, you have to delete the `KafkaNodePool` hosting the KRaft controllers and then apply the `disabled` value annotation in order to allow the operator to configure brokers not being connect to the KRaft controller quorum anymore but using ZooKeeper again and electing the new controller among them.

TBD: showing Kafka custom resource with `strimzi.io/kraft` as disabled and shell with all states during rollback finalization
...
It means that users should move soon to create new cluster by using only KRaft.
