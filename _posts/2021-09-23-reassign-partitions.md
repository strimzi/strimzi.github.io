...
Scaling of Kafka Brokers, removal of old brokers and increase in topics/partitions is pretty common to Kafka Users.
All these cases can lead to disbalance of load on the brokers, so to manage this, the most convenient way would be to reassign the partition among all the brokers.
...
`--generate`
It takes a set of topics and brokers and generates a reassignment JSON file which will result in the partitions of those topics being assigned to those brokers. 
Because this operates on whole topics, it cannot be used when you only want to reassign some partitions of some topics.

`--execute`
This takes a reassignment JSON file and applies it to the partitions and brokers in the cluster. 
...
If the reassignment is complete, --verify also removes any traffic throttles (--throttle) that are in effect. Unless removed, throttles will continue to affect the cluster even after the reassignment has finished.
...
1. There can be cases when you want to shutdown/remove your old broker pod from the cluster and start with some new broker pod.
   In this scenario the new pod won't get the partitions assigned to it. So you will have to reassign the
   partition among all the brokers.

2. It can also be used at times when you want to expand you cluster.
   You can add more brokers to your clusters and after that use the reassignment partition tool to assign the
...
Here is an example topic configuration. You can change it as per the requirement
