...
Now that we have a `GatewayClass` which is properly configured by an Envoy Gateway `EnvoyProxy` resource, we can create our `Gateway`. As discussed earlier in the setup overview, we want the Envoy Proxy pods to listen on port `9092`. That listener will be configured to have Passthrough TLS.
