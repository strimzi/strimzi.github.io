...
The node address and node port number give you all what is needed to connect to your cluster:

```
bin/kafka-console-producer.sh --broker-list <node-address>:<node-port> --topic <your-topic>
```

Fore more details, see the [Strimzi documentation](https://strimzi.io/docs/latest/full.html#proc-accessing-kafka-using-loadbalancers-deployment-configuration-kafka).
...
The overrides can specify either a DNS names or IP addresses.
This solution is not optimal, since you would need to maintain the addresses in the custom resource and remember to update them every time you are for example scaling up your Kafka cluster.
But in many cases you might not be able to change the addresses reported by the Kubernetes APIs.
So this gives you at least some way how to get it working.

Another thing which might make using node ports complicated are firewalls.
...
By default, the node port number are generated / assigned by the Kubernetes controllers.
...
Compared to other solutions such as load-balancers, routes or ingress there is no middleman which could be the bottleneck or which could add latency.
...
Node ports are very low level solution.
Often you will run into problems with the detection of the advertised addresses as describe in one of the chapters above.
