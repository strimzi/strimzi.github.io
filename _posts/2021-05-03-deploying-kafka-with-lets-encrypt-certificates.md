...
When deploying Apache Kafka on Kubernetes, Strimzi makes it is easy to configure Kafka listeners with TLS encryption.
...
When clients try to connect to a listener secured with such certificate, they will not trust it by default.
...
That brings a lot of complications since you need to get the public key, distribute it to all the clients and configure them to use it.
And you will need to update the public key when it changes for example because the old one expired.
This can be much easier if the listeners would use TLS certificates signed by a certification authority which the clients would trust out of the box.
...
The HTTP Challenge uses HTTP protocol to prove that you own the domain.
It requires you to put on some path on that domain some specific string which it will try to access using HTTP protocol.
For example, when requesting certificate for domain _example.com_ it will ask you to place a file with some specific information on the HTTP address `https://example.com/.well-known/acme-challenge/...`.
If you are able to do it, you have proven that you control the domain.
This works well for web applications which use the HTTP protocol.
But it doesn't work so well for applications using TCP protocol such as Apache Kafka.

Apache Kafka does not use the HTTP protocol so it cannot easily provide any information on given HTTP path.
...
It will request us to create some DNS record with some specific information to prove the domain ownership.
Using the DNS challenge makes it a bit more complicated for us, because we will need to give cert-manager access to the DNS management of our domain.
...
So to make the certificates work, we need to make sure that the clients connecting to the Kafka cluster are able to use the same domain names as will be in the certificates.
...
In the examples used in this blog post we will access Kafka using [Kubernetes NGINX Ingress Controller](https://kubernetes.github.io/ingress-nginx/) and we will use ExternalDNS to assign DNS names to the `Ingress` resources created by Strimzi and Kubernetes.

## DNS Management

Both cert-manager and ExternalDNS can work with many different DNS providers.
I personally use [Amazon AWS Route53](https://aws.amazon.com/route53/) for most of my domains and therefore it will be used through this blog post.
If you use some other provider, you will need to configure the cert-manager and ExternalDNS tools a bit differently.
...
To allow cert-manager and ExternalDNS communicate with the Route 53 APIs, we will need to give them the rights to do so.
...
In my case, I simply created two new users, attached the policies to them and stored the access key ID and secret access keys in Kubernetes Secrets.
...
But in addition to it, I configured the AWS credentials to be passed from the Secret as environment variables into the ExternalDNS deployment:
...
You should be also able to use different ways to pass the AWS credentials to cert-manager and ExternalDNS.
...
`ClusterIssuer` resource is cluster-scoped and it can be used from applications running in all namespaces in your cluster.
...
The `Issuer` resource would be the same, just have different `kind`.
...
The `Certificate` resource should be created in the same namespace where we later create the Kafka cluster.
...
* In the `annotations` section, I use the `external-dns.alpha.kubernetes.io/hostname` annotation to tell ExternalDNS that this `Ingress` resource should be assigned given DNS name.
  And ExternalDNS will automatically set-up the Route 53 DNS records.
  You of course do not have to use only `Ingress` - it can be used the same way also for load balancers or node ports.
* The `host` field configures the `Ingress` resource to expect the connection use this hostname when connecting to the broker.
...
Once the Kafka cluster is deployed, we can do a simple test if it uses the right certificate using OpenSSL.
...
We can now also test that we can produce and consumer messages from the Kafka cluster without configuring any certificate in our clients.
When using the consoler consumer or producer from Kafka, we need to specify the `--bootstrap-server` option and the `--topic` option.
...
You will not need to worry about anymore copying the self-signed certificates to your applications.
And in many cases, cert-manager and ExternalDNS will come handy also for other applications, not just for Apache Kafka.
