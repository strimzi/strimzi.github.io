...
```shell
oc apply -f https://github.com/strimzi/strimzi-kafka-operator/releases/download/0.11.2/strimzi-cluster-operator-0.11.2.yaml -n myproject
```
...
oc apply -f https://raw.githubusercontent.com/strimzi/strimzi-kafka-operator/0.11.2/examples/kafka/kafka-persistent-single.yaml -n myproject
```
...
```shell
oc run kafka-producer -ti --image=strimzi/kafka:0.11.2-kafka-2.1.0 --rm=true --restart=Never -- bin/kafka-console-producer.sh --broker-list my-cluster-kafka-bootstrap:9092 --topic my-topic
```

And to receive them:

```shell
oc run kafka-consumer -ti --image=strimzi/kafka:0.11.2-kafka-2.1.0 --rm=true --restart=Never -- bin/kafka-console-consumer.sh --bootstrap-server my-cluster-kafka-bootstrap:9092 --topic my-topic --from-beginning
```
